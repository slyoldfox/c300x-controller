...
module.exports = class Api {
	...
	handle(request, response, url, q) {
		if(!q.raw || q.raw !== "true" ) {
			...
			if(q.domain && q.service && q.entities) {
				...
				config.homeassistant.pages?.forEach((page) => {
					const dataItem = {}
					q.items.toString().length > 0 ? q.items.toString().split(',').map((item) => {
						...
						try {
							var fu = this[apiName]
							if(fu) {
								calls.push(fu.apply(this, [page, functionName, dataItem ]))
							} else {
				...
				Promise.all(calls).then( () => {
					...
					if(pages.length == 0 ) {
						pages.push({ badges: [ { "state": "!!! Fix !!!\nconfig.json" } ]})		
					}
					responseItem["data"] = { "pages": pages}
	...
	_api_badges(page, name, responseItem) {
		return this.#buildEntityState(page.badges, name, responseItem, (entity, sensor) => {
			 return { "state": this.#parseEntityState(entity, sensor["state"]) }
		})
	}

	_api_switches(page, name, responseItem) {
		return this.#buildEntityState(page.switches, name, responseItem, (entity, sensor) => {
	...
	_api_buttons(page, name, responseItem) {
		return this.#buildEntity(page.buttons, name, responseItem)
	}

	_api_images(page, name, responseItem) {
		return this.#buildEntity(page.images, name, responseItem)
	}
