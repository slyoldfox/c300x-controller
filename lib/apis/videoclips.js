...
module.exports = class Api {
	...
	handle(request, response, url, q) {
		...
		fs.readdirSync(utils.MESSAGES_FOLDER).forEach(file => {
			let resolvedFile = path.resolve(utils.MESSAGES_FOLDER, file);
			let stat = fs.lstatSync(resolvedFile)
			if (stat.isDirectory()) {
				let iniFile = utils.MESSAGES_FOLDER + file + "/msg_info.ini"
				var info = ""
				var vmMessage = ""
				var time = undefined
