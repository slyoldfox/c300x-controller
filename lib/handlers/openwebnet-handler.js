...
class OpenwebnetHandler {
	...
	switch( msg ) {
		...
		/*
		this.registry.foreachEndpointIp( (ipInHashForm) => { 
			console.log("should send stream to " + ipInHashForm )
			this.btAvMedia.addVideoStream(ipInHashForm)
			this.btAvMedia.addHighResVideoStream(ipInHashForm)
			this.btAvMedia.addAudioStream(ipInHashForm)
		 } )
		*/
		break
            case '*8*1#1#4#21*10##':
