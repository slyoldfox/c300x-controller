...
const debug = utils.getDebugger("openwebnet")
...
class OpenWebnet {
    ...
    run(resolve, reject) {
        ...
        this.client.setTimeout(3000, () => { debug('\t\tDL> [timeout connecting to openserver]'); this.client.end(); this.client.destroy() });
        this.client.on('error', (err) => { console.error(err); this.client.destroy() })
        this.client.once('connect', () => { debug('\t\tDL> [connected]') })
        this.client.on('data', (data) => { this.#data(data) })
        this.client.on('close', () => { debug('\t\tDL> [closed]'); })
            ...
            if (this.#commands.length > 0) {
                ...
                if (shouldcontinue) {
                    ...
                    if (command.callbacks.length == 0) {
                        ...
                        this.#commands.shift()
                        if (this.#commands.length > 0) {
                            debug("\t\tS -> " + this.#commands[0].command)
                            this.client.write(this.#commands[0].command)
                        }
                    }
                } else {
                    this.#failureCount++
                    debug("failure of (" + this.#failureCount + ") reached. Result: " + result + " does not pass function: " + p)
                    if (this.#commands.length > 0) {
                        debug("\t\tS -> " + this.#commands[0].command)
                        this.client.write(this.#commands[0].command)
                    }
