...
const { Mounts, Mount } = require('rtsp-streaming-server');
    ...
    createClientServer(mounts) {
        const clientServer = new ClientServer(6554, mounts, {
            checkMount: (req) => {
                ...
                const snapshotRequest = req.uri.toString().indexOf("/doorbell-video") >= 0
                const needsSipCall = (this.#inviteRequestHandler.incomingCallRequest && !snapshotRequest) || !this.#inviteRequestHandler.incomingCallRequest

                if (!needsSipCall) {
                    // If we are request a snapshot of the videostream, we don't need to setup a call if it's an incoming call
                    debug(`RTSP: NOT needing SIP call snapshotRequest? ${snapshotRequest} / incomingCallRequest: ${this.#inviteRequestHandler.incomingCallRequest}`)
                    return true
                } else {
                    debug(`RTSP: NEEDING SIP call snapshotRequest? ${snapshotRequest} / incomingCallRequest: ${this.#inviteRequestHandler.incomingCallRequest}`)
                }

                if (!this.#sipCallManager.hasActiveCall) {
                    debug("RTSP: SIP: calling and/or waiting")
...
module.exports = {
    create(registry,eventbus) {
        ...
        clientServer.server.on("request", (req, res) => {
            console.log(`========== REQUEST: ${req.uri} / Method: ${req.method} ==========`)
        ...
        const setupRequest = clientServer.setupRequest
        clientServer.setupRequest = (req, res) => {
            // Fix the status code return 
            ...
            return setupRequest.apply(clientServer, [req, res])
        }            
