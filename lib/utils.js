...
const CONF_XML = '/var/tmp/conf.xml';
...
function request(method, url, token, postData) {
    return new Promise(function (resolve, reject) {
        ...
        var req = (url.toString().indexOf("https://") >= 0 ? https : http).request(options, (res) => {
            var body = '';
            ...
            res.on("end", () => {
                if(res.statusCode >= 400 ) {
                    // Might need to finetune this
                    reject(`Request for url ${url} failed: ${body}`)
                } else {
                    resolve(body)
                }
            })
...
module.exports = {
    ...
    reloadUi() {
        ...
        const time = new Date();
        try {
            fs.utimesSync(CONF_XML, time, time);
        } catch (e) {
            let fd = fs.openSync(CONF_XML, 'a');
            fs.closeSync(fd);
        }        
    ...
    model() {
        if( !this["_model"] ) {
           if(fs.existsSync("/home/bticino/cfg/extra/.bt_eliot")) {
            this["_model"] = "c100x"
           } else if(fs.existsSync("/home/bticino/cfg/extra/")) {
            this["_model"] = "c300x"
           } else {
            this["_model"] = "unknown"
           }
    ...
    firewallAllowLAN() {
        try {
            // Detect if INPUT rules are active
            let output = child_process.execSync("iptables -L INPUT", {timeout: 2500}).toString().split('\n')
            if( output.length > 3 ) {
                // Fetch the LAN subnet from the route wlan0 table
                let subnet = child_process.execSync("/sbin/ip route show dev wlan0 | /bin/grep src").toString().split(' ')[0]
                console.log("*** Firewall is enabled, adding an ACCEPT rule for subnet: " + subnet)
                try {
                    // Check if a firewall rule already exists
                    child_process.execSync(`/usr/sbin/iptables -C INPUT -s ${subnet} -j ACCEPT`, {timeout: 2500})
                } catch(e) {
                    if(e.status == 1) {
                        // Insert a firewall rule on top of the list of the LAN
                        child_process.execSync(`/usr/sbin/iptables -I INPUT 1 -s ${subnet} -j ACCEPT`, {timeout: 2500})
                        console.log("*** Inserted firewall rule for subnet: " + subnet)
                    }
                }
            }            
        } catch( e ) {
            console.error("Failure querying firewall.")
        }        
    },    
    domain() {
    ...
    matchStringInFile(filename, lineMatcher, errorHandler) {
        return matchStringInFile(filename, lineMatcher, errorHandler)
    },
    requestGet(url, token) {
        return request('GET', url, token)
    },
    requestPost(url, postData, token) {
        return request('POST', url, token, postData)
    }
}
