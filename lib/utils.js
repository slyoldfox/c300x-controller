...
module.exports = {
    ...
    verifyFirewall() {
        try {
            // For now, bail out when iptables is not disabled since it causes pairing issues
            let output = child_process.execSync("/usr/sbin/iptables -L INPUT", {timeout: 2500}).toString().split('\n')
            if( output.length > 3 ) {
                console.error("\n\t!!! Your firewall is not disabled, which is required to use mDNS and Homekit.\n\t!!! Without it, pairing would fail. Aborting startup.")
                process.exit(1)
            }
        } catch( e ) {
